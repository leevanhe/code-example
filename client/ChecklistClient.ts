...
import ChecklistTemplateType from '@/dossier/constants/ChecklistTemplateType';
import { DossierId } from '@/dossier/models/AbstractDossier';
import { JobId } from '@/jobs/models/AbstractJob';
import { formatMoment } from '@/shared/converters/dateConverters';
...
import { ChecklistSectionId } from '../models/ids';
...
export default class ChecklistClient {
  ...
  static async addDamagePoint(context: ChecklistItemContext, damagePoint: ChecklistDamagePoint): Promise<ChecklistDamagePointId> {
    const response = await axios.post(`/api/mobopro/v1/dossiers/${context.dossier.id}/jobs/${context.item.id}/damagepoints`, {
  ...
  static async removeDamagePoint(damagePointId: ChecklistDamagePointId, itemId: JobId, dossierId: DossierId): Promise<void> {
