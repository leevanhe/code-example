<template>
  <CForm :data="formData" class="c-score-checklist-item-form">
    <CField path="score" v-slot="{value, onInput}">
      <CChecklistScore :value :options @input="onInput"/>
    </CField>

    <CField path="isNotApplicable" v-slot="{value, onInput}">
      <CChecklistItemNotApplicableButton :value @input="onInput"/>
    </CField>
  </CForm>
</template>
...
<script setup lang="ts">
  ...
  watch(() => props.context.item.result as any, v => formData.score = v);
