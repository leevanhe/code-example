...
import serviceEvents, { DiagnosticRemoved, DiagnosticCreated, JobTagsUpdated, MediaRemoved, MediaUploaded } from '@/shared/services/events/serviceEvents';
...
class ChecklistService {
  ...
  startEventListeners(): this {
    ...
    serviceEvents.on(DiagnosticCreated, e => this.updateDiagnosticCount(e));
  ...
  async addDamagePoint(point: ChecklistDamagePoint, context: ChecklistItemContext): Promise<void> {
    point.id = await ChecklistClient.addDamagePoint(context, point);
  }

  async removeDamagePoint(point: ChecklistDamagePoint, context: ChecklistItemContext): Promise<void> {
    await ChecklistClient.removeDamagePoint(present(point.id), context.item.id, context.dossier.id);
    const selectedItem = this.selectItem(context.checklist, context.item);
    removeById(selectedItem.damagePoints, point.id);
  }
  ...
  updateDiagnosticCount(event: DiagnosticCreated | DiagnosticRemoved): void {
    const item = this.getItem(event.job.id);
    if (!item) return;
